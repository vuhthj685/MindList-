...
import { getData, getConfig, storeData, getFileData } from '@/api'
...
export default {
  ...
  data() {
    return {
      ...
      showDragMask: false,
      // WeakMap 存储每个节点的 NodeCheckbox Vue 实例，key=节点对象，value=Vue实例
      // WeakMap 保证节点对象被GC回收时，关联条目自动清除，不会内存泄漏
      _checkboxInstanceMap: null
    }
  },
  computed: {
    ...mapState({
  ...
  mounted() {
    // 初始化 WeakMap（这里初始化而不是在 data 里，避免 Vue 响应式化 WeakMap）
    this._checkboxInstanceMap = new WeakMap()
    showLoading()
    this.getData()
    this.init()
    ...
    // 监听清单修改事件，同步到思维导图
    this.$bus.$on('mindmap_sync_from_checklist', this.onChecklistChanged)
    this.webTip()
  },
  beforeDestroy() {
    ...
    this.$bus.$off('mindmap_sync_from_checklist', this.onChecklistChanged)
    // 不需要遍历 WeakMap，WeakMap 会在节点对象被 GC 时自动清理
    this._checkboxInstanceMap = null
  ...
  methods: {
    ...
    // 清单修改后，同步到思维导图
    onChecklistChanged(fileId) {
      // 如果修改的是当前正在编辑的文件，重新加载数据
      if (fileId === this.fileId && this.mindMap) {
        const data = getFileData(this.fileId)
        if (data && data.root) {
          this.mindMap.setData(data.root)
        }
      }
    },
    ...
    init() {
      ...
      this.mindMap = new MindMap({
        ...
        createNodePostfixContent: (node) => {
          ...
          // 销毁旧的 Vue 实例，防止多个实例同时存在导致点击事件混乱
          if (this._checkboxInstanceMap) {
            const oldInstance = this._checkboxInstanceMap.get(node)
            if (oldInstance) {
              try {
                oldInstance.$destroy()
              } catch (e) {
              ...
              this._checkboxInstanceMap.delete(node)
            }
          }
          
          // 创建新的 Vue 组件实例
          const Component = Vue.extend(NodeCheckbox)
          ...
          // 用 WeakMap 存储实例引用（比 node._xxx 更安全）
          if (this._checkboxInstanceMap) {
            this._checkboxInstanceMap.set(node, instance)
          }
          
          // 获取字体大小来计算方框尺寸
          const fontSize = node.getStyle('fontSize', false) || 14
          const checkboxSize = Math.round(fontSize * 1.1)
          // 左右各留 0.3 个字符的间距（统一系数，避免计算不一致）
          const sideGap = Math.round(fontSize * 0.3)
          // 总宽度 = 左间距 + 方框宽度 + 右间距
          // 注意：多加 2px 给边框留空间，避免边框被 SVG 裁剪
          const totalWidth = sideGap + checkboxSize + sideGap + 2
          
          // 返回元素和尺寸（height = checkboxSize，由 simple-mind-map 自动垂直居中）
          return {
            el: instance.$el,
            width: totalWidth,
            height: checkboxSize
